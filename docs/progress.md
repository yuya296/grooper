...
| 0014 | Regex capture group expansion | 要確認 | regexキャプチャ（`$1`, `$<name>`, `$$`）のgroup展開とglobモード禁止バリデーションを実装済み。`pnpm test`/`pnpm build`通過、手動確認待ち |

## 現在のフォーカス
- PBI-0014の最終確認: regexキャプチャ展開（`$1`, `$<name>`）とglob時エラーを実拡張で手動確認する。
