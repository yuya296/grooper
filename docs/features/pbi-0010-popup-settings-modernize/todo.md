...
- [x] 実装: 保存フロー整備（差分時のみ保存有効、保存まで非反映、離脱警告）
- [x] テスト: 既存テスト更新と回帰確認
...
- 2026-02-07: import/export機能をUIから削除し、spec/機能要件ドキュメントを現仕様に更新。
- 2026-02-07: 保存フローを厳密化（差分検知で保存活性制御、未保存時の非反映ポリシー明示、離脱警告追加）。
- 2026-02-07: ルール追加フローを変更（`＋` で即追加せず、右ドロワー必須入力後の保存で一覧追加）。
- 2026-02-07: ルール編集ドロワーのPatternに正規表現バリデーションを追加（不正時は保存不可）。
- 2026-02-07: PBI-0010をクローズ（進捗台帳を完了へ更新）。
